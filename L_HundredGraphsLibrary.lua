module ("L_HundredGraphsLibrary", package.seeall)

------
--
-- EventWatcher2 - supplementary modules
--
...
local function Chart (chartType)
  ...
  local function draw (dataTable, options, extras)  
    this.setDataTable (dataTable)
    this.setOptions (options)
    return this.draw (extras)
  end 
  return {draw = draw}
end
...
-- (c) 2013,2014  AK Booer
  ...
  local function encode (Lua)
    ...
    local function object (x, index)  
      ...
      return table.concat {'{', table.concat (items, ','), '} '}
    end
