...
function LogHG(text)
	local function logcall(text)
		luup.log('[HundredGraphs Logger] ' .. (text or "empty"))
	end
	xpcall( logcall, errorhandlerHG, text )
end
...
function UpdateStartHG(lul_device, lul_service, lul_variable, lul_value_old, lul_value_new)  
  ...
  lul_value_old = lul_value_old or 'empty'
  lul_value_new = lul_value_new or 'empty'
...
local function GetCurrentEvents(lastfull, current)
  ...
  if (DEBUG == 1) then LogHG("GetCurrentEvents collected vars: " .. count) end
...
local function sendRequestHook(sender, current, lastnewHG, lastfull, payload, interval, DEBUG)
	...
	-- local response_body2 = {}
	local res, res1, res2, response_headers, status = 'waiting', monitors  
	local code = 0
	local code1 = 0
	local code2 = 0 
	local body = {}
...
function HGTimer()
  ...
  --lastfull = current
	...
	if (current - lastfull >= 60*60*2) then  
		...
		count = GetCurrentEvents(lastfull, current)
		LogHG('HGTimer getting GetCurrentEvents: ' .. #itemsExtendedHG .. '/' .. count)
		lastfull = current
		...
		count = GetNewEvents(lastnewHG, current)
		LogHG('HGTimer getting GetNewEvents: ' .. #itemsExtendedHG .. '/' .. count)
	end
...
function startup(lul_device)
	...
	if (deviceData == "" or deviceData == '-') then
		...
		LogHG(" Started with no devices, version: " .. version .. ' interval: ' .. interval)
	else
		VARIABLES = splitTable(deviceData)
	end
